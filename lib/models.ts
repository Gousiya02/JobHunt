...
export interface ISeekerProfile extends Document {
  email: string; // primary identity from Google Sign-In
  ...
  languages: string[];
  preferredLanguage: string; // Seeker's native/preferred language (e.g. English, Hinglish, Hindi, Kannada, Tamil, etc.)
  photo?: string;
  createdAt: Date;
}
...
export interface ICompanyProfile extends Document {
  ...
  ownerPhone: string;
  ownerEmail: string; // primary owner identity matching Google Sign-In email
  preferredLanguage: string; // Company owner's native/preferred language
  isVerified: boolean;
  createdAt: Date;
}
...
export interface IApplication extends Document {
  seekerEmail: string; // primary identity matching Google Sign-In email
  jobId: mongoose.Types.ObjectId;
  resumeText: string; // Original AI pitch in seeker's language
  translatedResumeText?: string; // Translated AI pitch in company owner's language
  fitScore?: number; // Suitability score (0-100) calculated by AI/proximity
  fitExplanation?: string; // One-sentence explanation of suitability
