...
exports.generateAIResume = generateAIResume;
...
async function generateAIResume({ seeker, job, pitchLanguage = 'English' }) {
...
async function generateFitAnalysis(seeker, job, distanceKm, companyLanguage = 'English') {
...
function generateMockFitAnalysis(seeker, job, distanceKm, companyLanguage) {
    let score = 90;
    if (distanceKm > 10)
        score -= 25;
        ...
        score -= 5;
    const matchedSkillsCount = seeker.skills.filter((s) => job.requiredSkills.includes(s)).length;
    if (matchedSkillsCount === 0 && job.requiredSkills.length > 0)
        score -= 15;
    else
        score += matchedSkillsCount * 3;
...
function generateMockResume(seeker, job, pitchLanguage) {
